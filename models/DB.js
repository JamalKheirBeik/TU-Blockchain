...
class DB {
    ...
    async init() {
        ...
        const [rows, fields] = await this.connection.execute("SELECT * FROM users WHERE isAdmin = 1");
        if (rows.length == 0) {
            ...
            await this.insert_user(keyPair.publicKey, keyPair.privateKey, process.env.ADMIN_EMAIL, process.env.ADMIN_PASSWORD, 1000000000, 1);
        }
        const transactions_table_query = "CREATE TABLE IF NOT EXISTS transactions (ID INT(10) AUTO_INCREMENT PRIMARY KEY, User_ID INT(10), Sender VARCHAR(2000) NOT NULL, Receiver VARCHAR(2000) NOT NULL, Amount INT(10) NOT NULL, Created_At TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT UT_ID FOREIGN KEY (User_ID) REFERENCES users(ID) ON DELETE CASCADE ON UPDATE CASCADE)";
        await this.create_table(transactions_table_query);
        const rewards_table_query = "CREATE TABLE IF NOT EXISTS rewards (R_ID INT(10) AUTO_INCREMENT PRIMARY KEY, U_ID INT(10), Hash VARCHAR(2000) UNIQUE NOT NULL, Reward INT(10) NOT NULL, Created_At TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT UR_ID FOREIGN KEY (U_ID) REFERENCES users(ID) ON DELETE CASCADE ON UPDATE CASCADE)";
    ...
    async insert_user(public_key, private_key, email, password, balance = 0, isAdmin = 0) {
    ...
    async update_user_balance(id, balance) {
        const query = `UPDATE users SET Balance = ${balance} WHERE ID = ${id}`;
        await this.connection.query(query);
    }
    async insert_transaction(sender_id, sender_public_key, receiver_public_key, amount) {
