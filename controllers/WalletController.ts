...
const user_signup_get = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.render("signup");
  res.redirect("/");
};

const user_signup_post = async (req: any, res: any) => {
  ...
  if (typeof response != "string" && "email" in response)
    return res.render("signup", { emailError: response?.email });
  if (typeof response != "string" && "password" in response)
    return res.render("signup", { passwordError: response?.password });
  res.render("signup", { privateKey: response });
};

const user_profile = async (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  let wallet = new Wallet();
  let response = await wallet.login(req.session.username, req.session.password);
  if (response.includes("wrong")) return res.redirect("/dashboard");
...
const user_login_get = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.render("login", { loggedIn: false });
  res.render("index", { loggedIn: true });
};

const user_login_post = async (req: any, res: any) => {
  if (req.session.loggedIn) return res.redirect("/");
  ...
  if (wallet.role != 0) return res.redirect("/");
...
const user_logout = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  req.session.destroy();
  res.redirect("/");
};

const user_dashboard = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  res.render("dashboard", { loggedIn: true });
};

const user_send_get = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  res.render("send", { loggedIn: true });
};

const user_send_post = async (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  let wallet = new Wallet();
  let login_response = await wallet.login(
    req.session.username,
    req.session.password
  );
  if (login_response.includes("wrong")) return res.redirect("/login");
...
const user_page_not_found = (req: any, res: any, next: any) => {
  res.status(404);
  res.send("404: File Not Found");
};

const user_mining = async (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  // start mining
  let wallet = new Wallet();
  let login_response = await wallet.login(
  ...
  res.render("mining", {
    ...
    isSuccess: true,
    message: mining_response,
    reward: Chain.instance.reward,
    newBalance: wallet.balance,
  });
};

const user_home = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.render("index", { loggedIn: false });
  res.render("index", { loggedIn: true });
};

const user_contact_get = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.render("contact", { loggedIn: false });
  res.render("contact", { loggedIn: true });
};

const user_contact_post = (req: any, res: any) => {
  res.send("hello");
  // if (!req.session.loggedIn) return res.render("contact", { loggedIn: false });
  // res.render("contact", { loggedIn: true });
};

const user_buy_get = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/login");
  res.render("buy", { loggedIn: true });
};

const user_buy_post = async (req: any, res: any) => {
...
const user_buy_success = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  res.render("success", { loggedIn: true });
};

const user_buy_cancel = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.redirect("/");
  res.render("cancel", { loggedIn: true });
};

const user_about = (req: any, res: any) => {
  if (!req.session.loggedIn) return res.render("about", { loggedIn: false });
